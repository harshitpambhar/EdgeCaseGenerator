...
import { HiOutlineBeaker, HiOutlineCode, HiOutlineChartBar, HiOutlineLightningBolt, HiOutlineClock, HiOutlineArrowRight, HiOutlineCloudUpload, HiOutlineDocumentReport, HiOutlineDownload, HiOutlineClipboardList } from 'react-icons/hi';
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, Tooltip } from 'recharts';
import AnalyticsCard from '../../components/dashboard/AnalyticsCard';
import RepoCard from '../../components/dashboard/RepoCard';
import CoverageChart from '../../components/dashboard/CoverageChart';
import { useAuth } from '../../context/AuthContext';
import api, { jobService, getErrorMessage } from '../../services/api';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    api.get('/requirements')
      .then((res) => {
        setRequirementsCount(Array.isArray(res.data) ? res.data.length : 0);
      })
      .catch(() => {});
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <Link to="/requirements" className="no-underline block">
          <motion.div
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-medium text-white">Functional Requirements</p>
                <p className="text-xs text-white/40 mt-1">Manage & map NLP constraints</p>
              </div>
              <div className="w-10 h-10 rounded-lg bg-emerald-500/20 flex items-center justify-center group-hover:bg-emerald-500/30 transition-colors">
                <HiOutlineClipboardList className="text-emerald-400 text-lg" />
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <AnalyticsCard title="Requirements parsed" value={requirementsCount.toLocaleString()} icon={HiOutlineClipboardList} color="rose" delay={0.15} />
      </div>
