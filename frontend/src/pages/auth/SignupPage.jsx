...
export default function SignupPage() {
  ...
  const inputClass = (field) =>
    `bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/20 focus-visible:ring-indigo-500/40 focus-visible:border-indigo-400/50 ${errors[field] ? 'border-rose-500/50' : ''}`;
    ...
    >
      ...
      <div className="rounded-xl bg-white/[0.03] border border-white/[0.08] p-7">
        <div className="mb-6">
          ...
          <p className="text-sm text-white/40 mt-1">Get started with TestGenAI for free.</p>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-5">
          <button type="button" className="flex items-center justify-center gap-2 h-9 rounded-lg bg-white/[0.04] border border-white/[0.08] text-sm text-white/70 hover:bg-white/[0.07] hover:text-white transition-colors cursor-pointer">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="name" type="text" value={name} onChange={e => setname(e.target.value)}
                placeholder="Jane Smith" className={`pl-9 ${inputClass('name')}`} />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="you@gmail.com" className={`pl-9 ${inputClass('email')}`} />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="password" type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                placeholder="Min. 6 characters" className={`pl-9 pr-9 ${inputClass('password')}`} />
          ...
          <p className="text-xs text-white/25 leading-relaxed">
            ...
            <a href="#" className="text-indigo-400 hover:underline no-underline">Terms of Service</a>{' '}
            and{' '}
            <a href="#" className="text-indigo-400 hover:underline no-underline">Privacy Policy</a>.
          </p>
        ...
        <p className="text-center text-xs text-white/30 mt-5">
          ...
          <Link to="/login" className="text-indigo-400 hover:text-indigo-300 no-underline transition-colors">Sign in</Link>
        </p>
      </div>
    </motion.div>
  );
